...
$(document).ready(function () { //מייצר ראש הדף
    ...
    subheader += '<button type="button" class="btn btn-outline-secondary box-shaded" onClick="liveReports()">Live reports</button>';
...
function createCard(symbol, name, id) {//מייצר קרטיסיה של מטבעה
    ...
    $('[name="' + id + '"]').bootstrapSwitch('size', 'mini', 'animate', true);
...
function createMiniCards() {//מייצר חלון עם שמות של מטבעות וכפתורים להסרתם. עובד בסנכרון עם דפ ראשי ולוקלסטורג׳ איפה מטבעות שנבחרו נשמרים
    ...
    function createMiniContent(id, symbol) {
        ...
        string += '</div><div><div>';
        string += '<label><input type="checkbox" class="minicheckbox" checked data-label-text="Live report" onchange="handleMiniSwitch(this, this.name)" name="' + id + '" id="minicheckbox_' + id + '"><span></span></label></div></div></div>';
        
        $('.modal-content').append(string);
        // $('[name="minicheckbox_' + id + '"]').bootstrapSwitch();
        $('#minicheckbox_' + id).bootstrapSwitch('size', 'small', 'animate', true, 'state', 'true', 'false');
        $('#minicheckbox_' + id).bootstrapSwitch('state', 'true', 'false');
...
function handleMiniSwitch(checkbox, checkedCoin) {//במקרא של בחירת המטבעה בודק תנאים ושמר בלוקלסטורג׳ או מוחק ממנו
...
function aboutMe() {//מייצר כרטיסיה עם מידה עבורי
    var string = '<div class="container col-sm-6 align-items-center shadow p-3 mb-5 bg-white rounded"><div class="container"><h3 class="text-center"><strong>Alexander Bruder</strong></h3>';
    ...
    string += '<div class="card-header shadow p-3 mb-5 bg-white rounded"><h6 class="text-center"><strong>Contact me:</strong></h6><p class="card text-center">Mobile:</br><a href="tel:+972548887511">+972548887511</a></p><p class="card text-center">E-mail:</br><a class="text-center" href="mailto:nive.bald.man@gmail.com">nice.bald.man@gmail.com</a></p></div></div></div>';
    $('.for_insert').empty();
    $('.for_insert').append(string);
...
function setSwitchOn(id) {//כפתור במצב ON
    if ($('#checkbox_' + id).prop("checked") == true) {
        ...
        $('#checkbox_' + id).bootstrapSwitch('state', true);
    }
...
function setSwitchOff(id) {//כפתור במצב OFF
    if ($('#checkbox_' + id).prop("checked") == true) {
        ...
        var i = setInterval(function () {
            ...
            if (counter === 1) {
                clearInterval(i);
                $('#checkbox_' + id).bootstrapSwitch('state', false);
...
function checkLocalstorage() {//בודק לוקלסטורג׳ לאחר שמטבעות הוצגו ומזיז את הכפתורים של המטבעות שנבחרו
    if (localStorage.length > 0) {
        ...
        while (i--) {
            tempArray.push(localStorage.getItem(keys[i]));
        ...
        for (let i = 0; i < tempArray.length; i++) {
            ...
            $('#checkbox_' + tempArray[i]).bootstrapSwitch('state', 'true', 'false');
